...
import { permanentlyDeleteDevice, restoreDevice, updateDeviceNotes } from "@/app/actions";
...
export default async function DeviceDetailPage({ params, searchParams }: PageProps<"/devices/[id]">) {
  ...
  const device = await prisma.asset.findUnique({ where: { id }, include: { customer: { include: { locations: { orderBy: { code: "asc" } } } }, location: true, repairIntakes: { orderBy: { pickedUpAt: "desc" }, include: { pickup: true } } } });
  if (!device) notFound();
  ...
  return (
    <>
      <PageHeader eyebrow="Device" title={device.rawModel ?? `${device.brand} ${device.family ?? ""}`} action={<div className="flex items-center gap-2"><EditDeviceDialog device={{ id: device.id, brand: device.brand, rawModel: device.rawModel, family: device.family, modelYear: device.modelYear, chipset: device.chipset, serialNumber: device.serialNumber, assetTag: device.assetTag, locationId: device.locationId }} key={device.updatedAt.toISOString()} locations={device.customer.locations} /><Link className="button-ghost border-border" href="/devices"><ArrowLeft className="size-3.5" />All devices</Link></div>} />
      ...
      <div className="grid gap-4 xl:grid-cols-[320px_1fr]">
        ...
        <div className="space-y-4">
          <section className="panel overflow-hidden">
            <header className="border-b border-border bg-zinc-50 px-panel py-2.5"><h2 className="text-sm font-semibold">Device notes</h2><p className="text-2xs text-muted">General information that follows this device across repairs.</p></header>
            <form action={updateDeviceNotes.bind(null, device.id)} className="p-panel">
              <label className="sr-only" htmlFor="device-notes">Device notes</label>
              <textarea className="min-h-28 w-full resize-y rounded-md border border-border bg-white px-2.5 py-2 text-xs outline-none focus:border-primary focus:ring-2 focus:ring-blue-100" defaultValue={device.notes ?? ""} id="device-notes" maxLength={10_000} name="notes" placeholder="Add device-specific notes…" />
