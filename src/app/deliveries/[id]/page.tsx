...
import { addRepairToDelivery, addSaleDevice, deleteDraftDelivery, finalizeDelivery, removeRepairFromDelivery, removeSaleDevice, reopenDelivery } from "@/app/actions";
    ...
    <>
      ...
      <div className="grid gap-4 xl:grid-cols-[1fr_320px]">
        ...
        <aside className="space-y-3">
          ...
          {isDraft && <form action={finalizeDelivery.bind(null, delivery.id)} className="panel p-panel"><button className="button-primary w-full" disabled={!deviceCount} type="submit"><PackageCheck className="size-3.5" />Complete delivery</button><p className="mt-2 text-2xs leading-4 text-muted">Completing this delivery returns {delivery.repairs.length} repair device{delivery.repairs.length === 1 ? "" : "s"}{delivery.items.length ? ` and records ${delivery.items.length} new device${delivery.items.length === 1 ? "" : "s"}` : ""}. Removed repairs remain on their open pickups.</p></form>}
