...
export function AddSaleDeviceForm({
  ...
  return <form action={formAction} className="grid gap-3 p-panel sm:grid-cols-2" ref={formRef}>
    ...
    <div className="grid gap-1"><label className="text-xs font-medium" htmlFor="sale-device-serial">Serial number</label><span className="flex"><input className="control min-w-0 flex-1 rounded-r-none" id="sale-device-serial" name="serialNumber" ref={serialNumberRef} required /><BarcodeScannerButton onDetected={(value) => { if (serialNumberRef.current) serialNumberRef.current.value = value; }} /></span></div>
